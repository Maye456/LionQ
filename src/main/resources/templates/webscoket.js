    ...
    $.get(url + "/fetchAllUsers/"+userId, function (response) {
        ...
        for (let i = 0; i < users.length; i++) {
            console.log(users[i]['username'])
                ...
                usersTemplateHTML = usersTemplateHTML + '<li class="active" id="child_message" onclick="formMessageLaunch('+users[i]['user_Id']+',\''+users[i]['username']+'\',\'user\')" data-userid="'+users[i]['user_Id']+'" data-type="user">'+
                '<div class="d-flex bd-highlight">'+
                '<div class="img_cont">'+
                '<img class="rounded-circle user_img">'+
                '<span class="online_icon"></span>'+
                '</div>'+
                '<div class="user_info" id="userNameAppender_' + users[i]['user_Id'] + '">'+
